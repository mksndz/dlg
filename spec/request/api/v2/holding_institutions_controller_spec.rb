...
RSpec.describe 'API V2 for Holding Institutions', type: :request do
  ...
  context 'can get single record info using #show' do
    ...
    context 'associated collection info' do
      before :each do
        @c1 = Fabricate :collection_with_repo_and_item, public: true
        Fabricate :item, collection: @c1, repository: @c1.repository, portals: @c1.portals, holding_institutions: @c1.holding_institutions
        hi_no = Fabricate :holding_institution
        @c1.items.last.holding_institution_ids = [hi_no.id]
        @c1.items.last.save
      end
      it 'includes information about assigned public collections' do
        get "/api/v2/holding_institutions/#{@c1.holding_institutions.first.slug}.json",
            {},
            headers
        json = JSON.parse(response.body)
      ...
      it 'returns item counts for collections that only count items with the
        ...
        get "/api/v2/holding_institutions/#{@c1.holding_institutions.first.slug}.json",
            ...
            headers
        json = JSON.parse(response.body)
        expect(json['public_collections'][0]).to have_key'collection_institution_item_count'
        expect(json['public_collections'][0]['collection_institution_item_count']).to eq 1
      end
    end
  end
end